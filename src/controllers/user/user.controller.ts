...
export async function userLogin(
  ...
  try {
    ...
    const jwt_secret = fs.readFileSync("jwtsecret.txt", { encoding: "utf8" });
    ...
    res.status(200).json({
      ...
      token,
      tokenExpiration: expirationTime === "1h" ? 3600000 : 0,
    });
  } catch (error) {
...
export async function isAuntheticated(
  req: UserRequest,
  res: Response,
  next: NextFunction
) {
  ...
  try {
    ...
    const link = `${
      process.env.NODE_ENV === "production"
        ? process.env.UI_BASE_URL_PROD
        : process.env.UI_BASE_URL
    }/auth/password-reset/${user.username}/${token}`;
    const mailOptions = {
